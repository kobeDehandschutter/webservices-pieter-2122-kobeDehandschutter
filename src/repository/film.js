...
const findAll = ({ limit, offset }) => {
  return getKnex()(tables.filmsOrSeries)
    .select()
    .limit(limit)
    .offset(offset)
    .orderBy('name', 'ASC');
...
const findByName = (name) => {
  return getKnex()(tables.filmsOrSeries).where('name', name).first();
};
 ...
 */
const findById = (id) => {
  return getKnex()(tables.filmsOrSeries).where('id', id).first();
};
...
const findCount = async () => {
  const [count] = await getKnex()(tables.filmsOrSeries).count();
  return count['count(*)'];
};
...
const updateById = async (id, { name, rating }) => {
  try {
    await getKnex()(tables.filmsOrSeries)
      .update({
        name,
        rating,
      })
    ...
    const logger = getChildLogger('filmsOrSeries-repo');
...
const deleteById = async (id) => {
  try {
    const rowsAffected = await getKnex()(tables.filmsOrSeries)
      .delete()
      .where('id', id);
